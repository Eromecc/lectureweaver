...
- Live-analysis timeouts are bounded independently at the provider (120 seconds), browser (170 seconds), and function (180 seconds) layers. Live output is capped at 8,000 tokens without optional Anki cards and 10,000 tokens with cards; the prompt also constrains and merges the requested material so those ceilings do not act as silent truncation. Timeout recovery preserves the validated source map and is manual; the application does not silently repeat a potentially billable request.
