...
Live analysis has no application-imposed browser deadline: the page keeps waiting for the server unless the browser, network, or hosting platform ends the connection. On Vercel Hobby, the analysis function uses the available five-minute ceiling. The upstream provider guard runs at 285 seconds, leaving a short margin for the function to return a typed timeout before the 300-second platform limit. A timeout keeps the local source map available, so the user can retry the same map or choose another ready provider without parsing the files again. LectureWeaver never retries a paid model request automatically because an interrupted request may still have consumed provider resources; every retry is a deliberate user action and may incur a new charge.
...
- A live request sends normalized chunks to the same-origin analysis route. The selected AI provider receives source text, structural IDs, source type, and optional heading context; application-owned filenames and trusted locators are omitted from the model prompt and reattached only from the local source map. Users should still treat live analysis as a transmission of source text to the selected provider.
...
- Provider output must pass the strict wire Zod schema, the domain schema, duplicate/reference checks, change-type rules, output-option rules, and artifact-specific evidence rules. DeepSeek JSON Output receives a bounded local compatibility pass first; it may normalize harmless representation drift and derives redundant section/card evidence only from unchanged assessment evidence. Invalid, unsafe, unknown-evidence, or truncated output still fails closed.
...
- Live analysis has no application-imposed browser timeout. The upstream provider request is guarded at 285 seconds and the Vercel Hobby function is capped at its 300-second platform ceiling. Live output is capped at 8,000 tokens without optional Anki cards and 10,000 tokens with cards; the prompt also constrains and merges the requested material so those ceilings do not act as silent truncation. Timeout recovery preserves the validated source map and is manual; the application does not silently repeat a potentially billable request.
...
- **DeepSeek:** OpenAI-compatible Chat Completions at `https://api.deepseek.com`, using JSON Output with `deepseek-v4-flash` by default. DeepSeek JSON Output guarantees parseable JSON, not schema adherence. LectureWeaver therefore canonicalizes only bounded format drift, derives section/card citations from the linked assessments' unchanged evidence, and then runs the same strict Zod and trusted-chunk semantic validation. This is local processing and never triggers a hidden second API request.
