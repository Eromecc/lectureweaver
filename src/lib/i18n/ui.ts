...
const ENGLISH_UI_MESSAGES = {
  ...
  "pipeline.sourceMapReady": "Source map ready",
  "pipeline.waitForProvider": "Waiting for provider",
  "pipeline.validateOutput": "Validate returned output",
  "pipeline.progressAria": "{kind} progress",
...
const SIMPLIFIED_CHINESE_UI_MESSAGES = {
  ...
  "pipeline.sourceMapReady": "来源地图已就绪",
  "pipeline.waitForProvider": "等待服务商返回",
  "pipeline.validateOutput": "验证返回结果",
  "pipeline.progressAria": "{kind}进度",
  "pipeline.elapsed": "已用时间 {time}",
...
const JAPANESE_UI_MESSAGES = {
  ...
  "pipeline.sourceMapReady": "ソースマップの準備完了",
  "pipeline.waitForProvider": "プロバイダーの応答待ち",
  "pipeline.validateOutput": "返された出力を検証",
  "pipeline.progressAria": "{kind}の進行状況",
  "pipeline.elapsed": "経過時間 {time}",
  "pipeline.checkingSafety": "ファイルの安全性を確認中…",
  "pipeline.analyzing": "{provider} · {model} で正規化済みチャンクを分析中…",
  "pipeline.liveWaitHint":
    "完了時刻の正確な目安はありません。ブラウザーが待機している間はこのタブを開いたままにしてください。長時間応答がない処理は、ホスティング環境または選択したプロバイダーによって停止される場合があります。",
...
const KOREAN_UI_MESSAGES = {
  ...
  "pipeline.sourceMapReady": "소스 맵 준비 완료",
  "pipeline.waitForProvider": "제공자 응답 대기 중",
  "pipeline.validateOutput": "반환된 결과 검증",
  "pipeline.progressAria": "{kind} 진행 상황",
  "pipeline.elapsed": "경과 시간 {time}",
  "pipeline.checkingSafety": "파일 안전성 확인 중…",
  "pipeline.analyzing": "{provider} · {model}(으)로 정규화 청크 분석 중…",
  "pipeline.liveWaitHint":
    "정확한 완료 예상 시간은 없습니다. 브라우저가 기다리는 동안 이 탭을 열어 두세요. 장시간 응답이 없는 작업은 호스팅 플랫폼이나 선택한 제공자가 중단할 수 있습니다.",
