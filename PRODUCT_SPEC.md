...
- Live analysis has no application-imposed browser deadline. The upstream provider request is guarded at 285 seconds and the Vercel Hobby application function uses its 300-second platform ceiling. The remaining margin allows cleanup and delivery of a typed timeout response before the hosting platform terminates the function.
...
- **DeepSeek:** use OpenAI-compatible Chat Completions JSON Output with thinking disabled for this extraction contract. Because JSON Output guarantees JSON syntax rather than LectureWeaver schema adherence and may return empty content, apply only bounded local canonicalization before strict parsing. Section/card evidence is deterministically rebuilt from unchanged linked-assessment evidence; unknown assessment evidence, unknown IDs, unsafe Markdown, mixed-status sections, and all other unrecoverable output still fail closed. Canonicalization never starts another provider request.
...
- `/api/analyze` and `/api/speech` accept validated JSON; `/api/transcribe` accepts only the bounded multipart audio contract. Every server route applies content/size validation, response-cache disabling, allowlisted targets/endpoints, explicit credential-mode checks, and secret-safe error mapping. Live analysis has no application-imposed browser timeout; its provider/function bounds are 285/300 seconds respectively. Audio routes retain their separate bounded provider contracts.
...
- artifact evidence must be a subset of the linked assessments' validated evidence and must overlap each linked assessment. For DeepSeek, redundant section/card references are deterministically canonicalized to the ordered union of those already-selected assessment references before this rule is checked.
